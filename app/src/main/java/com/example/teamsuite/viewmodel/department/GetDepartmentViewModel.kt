package com.example.teamsuite.viewmodel.department
...
class GetDepartmentViewModel(private var repository: GetDepartmentRepository): BaseViewModel() {
    val departmentData = MutableLiveData<GetDepartmentResponse>()
    ...
    fun getDepartment(){
        viewModelScope.launch {
            ...
            loading.value = true
            try {
                val response = repository.getDepartment()
                if (response.isSuccessful && response.body() != null){
                    departmentData.value = response.body()
                }
                else{
