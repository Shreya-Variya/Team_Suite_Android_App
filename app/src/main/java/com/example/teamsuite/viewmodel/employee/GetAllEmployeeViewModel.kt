...
class GetAllEmployeeViewModel(private val repository: GetAllEmployeesRepository): BaseViewModel() {
    val employeeData = MutableLiveData<GetAllEmployeeResponse>()
    val error = MutableLiveData<String>()
    val loading = MutableLiveData<Boolean>()

    fun getAllEmployee(companyid: String){
        viewModelScope.launch {
            setLoading(true)
            loading.value = true
            try{
                val response = repository.getAllEmployee(companyid)
