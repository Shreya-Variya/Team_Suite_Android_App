...
class GetEmployeeAttendanceViewModel(private val repository: GetEmployeeAttendanceRepository): BaseViewModel() {
    val employeeAttendanceData = MutableLiveData<GetEmployeeAttendanceResponse>()
    val error = MutableLiveData<String>()
    val loading = MutableLiveData<Boolean>()

    fun getEmployeeAttendance(companyid: String){
        viewModelScope.launch {
            setLoading(true)
            loading.value = true
            try {
                val response = repository.getEmployeeAttendance(companyid)
                if (response.isSuccessful && response.body() != null){
                    employeeAttendanceData.value = response.body()
                }
                else{
