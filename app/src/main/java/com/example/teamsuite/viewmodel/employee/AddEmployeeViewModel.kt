...
class AddEmployeeViewModel(private var repository: AddEmployeeRepository): BaseViewModel() {
    val addEmployeeResult = MutableLiveData<AddEmployeeResponse>()
    val error = MutableLiveData<String>()
    val loading = MutableLiveData<Boolean>()

    fun addEmployee(request: AddEmployeeWrapper){
        viewModelScope.launch {
            setLoading(true)
            loading.value = true
            try {
                val response = repository.addEmployee(request)
                if (response.isSuccessful && response.body() != null){
                    addEmployeeResult.value = response.body()
                }
                else{
