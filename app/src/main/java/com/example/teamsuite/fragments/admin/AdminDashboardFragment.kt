...
import com.example.teamsuite.databinding.FragmentAdminDashboardBinding
...
import com.example.teamsuite.viewmodelfactory.stats.AdminStatsViewModelFactory
...
class AdminDashboardFragment : BaseFragment() {
    ...
    private lateinit var adminStatsViewModel: AdminStatsViewModel
...
//        return inflater.inflate(R.layout.fragment_admin_dashboard, container, false)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        setProfileData()
        userData?.let {
            adminStatsViewModel.getTotalEmployees(it.companyId)
            adminStatsViewModel.onLeaveEmployees(it.companyId)
        }
        setupClickListeners()
        ...
        observeGetTotalEmployeesResponse()
        observeOnLeaveEmployeesResponse()
    }
    ...
    /*
    Function for set up the admin stats view model
     */
    private fun setupAdminStatsViewModel(){
        adminStatsViewModel = ViewModelProvider(
            this,
            AdminStatsViewModelFactory(AdminStatsRepository())
        )[AdminStatsViewModel::class.java]
    }
    ...
    /*
    Function for observe the get total employees response
     */
    private fun observeGetTotalEmployeesResponse(){
        adminStatsViewModel.getTotalEmployeeResult.observe(viewLifecycleOwner){ response ->
            if (response.success){
                hideLoader()
                binding.stats.txtTotalEmployee.text = response.totalEmployees.toString()
                binding.stats.txtPresentEmployee.text = response.totalPresentEmployees.toString()
                binding.stats.txtAbsentEmployee.text = response.totalAbsentEmployees.toString()
            }
            else{
                Toast.makeText(requireContext(), response.message, Toast.LENGTH_SHORT).show()
            }
        }
        adminStatsViewModel.error.observe(viewLifecycleOwner){ error ->
            Toast.makeText(requireContext(), error, Toast.LENGTH_SHORT).show()
        }
        adminStatsViewModel.isLoading.observe(viewLifecycleOwner){
            if (it) showLoader() else hideLoader()
        }
    }

    /*
    Function for observe the on leave employees response
     */
    private fun observeOnLeaveEmployeesResponse(){
        adminStatsViewModel.onLeaveEmployeeResult.observe(viewLifecycleOwner){ response ->
            if (response.success){
                hideLoader()
                binding.stats.txtLeaveEmployee.text = response.totalOnLeaveEmployees.toString()
            }
            else{
                Toast.makeText(requireContext(), response.message, Toast.LENGTH_SHORT).show()
            }
